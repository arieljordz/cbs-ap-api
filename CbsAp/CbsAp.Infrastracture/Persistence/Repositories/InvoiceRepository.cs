...
namespace CbsAp.Infrastracture.Persistence.Repositories
{
    ...
    // Due Date Range
    .AndIf(startDueDate.HasValue,
        s => s.DueDate >= startDueDate)

    .AndIf(endDueDate.HasValue,
        s => s.DueDate <= endDueDate)

    // Scan Date Range
    .AndIf(startScanDate.HasValue,
        s => s.ScanDate >= startScanDate)

    .AndIf(endScanDate.HasValue,
        s => s.ScanDate <= endScanDate)
    ...
    // Map ID
    .AndIf(!string.IsNullOrWhiteSpace(mapID),
        s => s.MapID == mapID)
 ...
 .AndIf(daystillDue.HasValue && daystillDue.Value != 0,
    s =>
        s.EntityProfile != null &&
        (
            // Based on ScanDate
            (s.EntityProfile.InvDueDateCalculation == 1 &&
             s.ScanDate != null && !string.IsNullOrEmpty(s.PaymentTerm) &&
             EF.Functions.DateDiffDay(
                 s.ScanDate.Value,
                 s.ScanDate.Value.AddDays(int.Parse(s.PaymentTerm!))
             ) - 1 == daystillDue.Value)
            ...
            (s.EntityProfile.InvDueDateCalculation == 2 &&
             s.CreatedDate != null && !string.IsNullOrEmpty(s.PaymentTerm) &&
             EF.Functions.DateDiffDay(
                 s.CreatedDate.Value,
                 s.CreatedDate.Value.AddDays(int.Parse(s.PaymentTerm!))
             ) - 1 == daystillDue.Value)
            ...
            (s.EntityProfile.InvDueDateCalculation == 3 &&
             s.InvoiceDate != null && !string.IsNullOrEmpty(s.PaymentTerm) &&
             EF.Functions.DateDiffDay(
                 s.InvoiceDate.Value,
                 s.InvoiceDate.Value.AddDays(int.Parse(s.PaymentTerm!))
             ) - 1 == daystillDue.Value)
        )
);
            ...
            sortField = sortDictionary.ContainsKey(sortField ?? string.Empty) ? sortDictionary[sortField ?? string.Empty] : null;
    ...
    .AndIf(!string.IsNullOrWhiteSpace(invRoutingFlowName),
        s => s.InvRoutingFlow != null &&
             s.InvRoutingFlow!.InvRoutingFlowName!.Contains(invRoutingFlowName!))

    // Next Role
    .AndIf(!string.IsNullOrWhiteSpace(nextRole),
        s =>
            s.InvInfoRoutingLevels!
                .Where(x => x.InvFlowStatus == 0)
        ...
        public async Task<InvoiceDto> GetInvoiceInfo(long invoiceID, CancellationToken token)
        {
            var invoice = await _dbcontext.Invoices
                ...
                .Select(x => new InvoiceDto
                {
                    ...
                    Note = x.Note,
                    ApproverRole = x.ApprovedUserInvoices != null ? x.ApprovedUserInvoices.UserID : string.Empty,
